...
def k_fold(x, y, rule_base, way, k):
    ...
    #
    # x_train, x_test = pd.DataFrame(train_X, columns=[0]), pd.DataFrame(test_X, columns=[0])
    # y_train, y_test = pd.DataFrame(train_y, columns=['y']), pd.DataFrame(test_y, columns=['y'])
    #
    # df_train = pd.concat([x_train, y_train], axis=1)
    # df_test = pd.concat([x_test, y_test], axis=1)
    #
    # ave_accuracy += run_EBRB(df_train, df_test, rule_base, way)
...
def get_accuracy(df_test, rule_base, way):
    ...
    for t_data in data:
        ...
        normalization_active_weight(rule_base.rules)
