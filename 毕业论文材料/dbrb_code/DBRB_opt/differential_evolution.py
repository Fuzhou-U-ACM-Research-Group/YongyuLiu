...
class MyProblem(ea.Problem):
    def __init__(self, brb_model, X, y):
        ...
        Dim = brb_model.get_params_num()['total']  # 决策变量个数
    ...
    def aimFunc(self, pop):
        ...
        index = 0
        for i in range(0, len_D * len_rules, len_D):
...
def run_DE_algorithm(brb_model, X, y, NIND=50, MAXGEN=200):
    """
    ...
    params = brb_model.partition_params(best_params)
    print('属性权重：%s' % params["attr_weight"])
    print('规则权重：%s' % params["rule_weight"])
    print('规则形式:')
    for f in params["rules_formation"]:
        print(f)
