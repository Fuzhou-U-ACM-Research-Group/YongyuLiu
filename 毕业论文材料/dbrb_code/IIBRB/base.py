...
def calc_active_weights(similarities, rules, n_features, sigma):
    ...
    for k in range(len(rules)):
        ...
        for j in range(n_features):  # alpha_i = alpha_{i-1} + (1 - alpha_{i-1}) * sigma_i * matchDegree_i
            if j == 0:
                alpha = sigma[j] * similarities[k][j]
            else:
                alpha = alpha + (1 - alpha) * sigma[j] * similarities[k][j]
...
def evidence_reasoning(active_weights, rules, D):
    ...
    for j in range(len(D)):
        for k in range(len(rules)):
            fir_para[j] *= (active_weights[k] * rules[k].beta[j] + 1 - active_weights[k] * np.sum(rules[k].beta))
        fir_para_sum += fir_para[j]

    sec_para, thr_para = 1.0, 1.0  # sigma(1-w(k))
    for k in range(len(rules)):
        sec_para *= (1 - active_weights[k] * np.sum(rules[k].beta))
        thr_para *= (1 - active_weights[k])

    for j in range(len(D)):
        beta[j] = (fir_para[j] - sec_para) / (fir_para_sum - (len(D) - 1) * sec_para - thr_para)
