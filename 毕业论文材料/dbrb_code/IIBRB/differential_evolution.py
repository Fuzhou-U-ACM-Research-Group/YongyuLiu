...
def run_DE_algorithm(brb_model, X, y, NIND, MAXGEN):
    """
    ...
    print('属性权重：%s' % brb_model.sigma)
    for i in range(len(brb_model.rules)):
        print('W: %20s || beta: %s' % (brb_model.rules[i].theta, brb_model.rules[i].beta))
    print('')
    return brb_model
